...
\author[秦非冉]{学生：秦非冉\\[5mm] 学号：2023010889}
...
\begin{frame}{jcode 是什么}
  \begin{block}{\small jcode：一个 TUI（终端用户界面）应用}
    \small
    \begin{itemize}
  ...
  \end{block}
  \centering
  \small\textbf{jcode 是一个"内核压力测试"---用到了几乎所有内核子系统}
\end{frame}
...
\begin{frame}{Fix 2：内核态 COW 缺页 panic}
  ...
  \begin{block}{修复}
    \begin{itemize}
      ...
      \item epoll/poll 已改用内核缓冲区 + \texttt{vm\_write\_slice} 安全写回
      \item select 仍使用 \texttt{get\_as\_mut()}（未改）
...
% epoll 事件通知（同一 section 下的 subsection）
...
% 异步任务调度（同一 section 下的 subsection）
...
% TTY 终端（同一 section 下的 subsection）
...
% ostool 鼠标转发（同一 section 下的 subsection）
...
\begin{frame}[fragile]{\footnotesize jcode 启动全链路（上）：修复在 QEMU 运行过程中的位置}
    \begin{lstlisting}[basicstyle=\ttfamily\tiny, frame=none, aboveskip=0pt, belowskip=0pt]
用户在 QEMU 终端输入 `jcode`
    |
    v
[文件系统] ext4 挂载
...
[TUI <-> serve] Unix socket IPC
  ...
  关闭时 EOF 信号顺序正确                (Fix 6e)\end{lstlisting}
\end{frame}

\begin{frame}[fragile]{\footnotesize jcode 启动全链路（下）：修复在 QEMU 运行过程中的位置（续）}
    \begin{lstlisting}[basicstyle=\ttfamily\tiny, frame=none, aboveskip=0pt, belowskip=0pt]
    |
    v
[serve 发出 HTTPS 请求]
...
[TUI 输出到终端]
  ...
  COW 缺页安全处理                       (Fix 2)\end{lstlisting}
\end{frame}
